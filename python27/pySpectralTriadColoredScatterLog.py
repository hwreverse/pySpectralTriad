...
resolution = 10000 # Maybe too much for your system?
...
logpr=interp1d([-10000,0,100000000],[1,1,100000001]) # done to prepare for log10 - log does not like numbers <= 0 !
...
def doPlot(): 
	...
	#plt.ylim(0,log10(defaultYLimit))
	plt.yscale('log')
	plt.title('Spectral Response')           
	plt.grid(True)                              
	plt.ylabel('Log(28.6 nW/cm2/count)')
	plt.xlabel('Wavelenght in nm')
	plt.xrange=[410,940]
	   
	plt.legend(loc='upper left')                

	plt.scatter(wLspace,logpr(f(mp(wLspace))), c=wLspace, norm=norm, cmap=cmap)
